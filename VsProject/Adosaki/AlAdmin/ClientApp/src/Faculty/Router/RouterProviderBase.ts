...
import {
    NavigateMatchModel,
    NavigateMatchDataModel,
} from '@/Faculty/Router/Models/NavigateMatchModel';
...
export default class RouterProviderBase {
    ...
    public ContentRender<T extends HeosabiComponent>({
        Page,
        Component,
    }: {
        ...
        return (match: NavigateMatchModel) => {
            if (
                null === GlobalStatic.PageLayout ||
                GlobalStatic.PageLayout.constructor.name !== Page.name
            ) {
                ...
                GlobalStatic.PageLayout.DomThisComplete = () => {
                    ...
                    if (!this.CurrentPageLayoutDom) {
                        GlobalStatic.app.DomThis.appendChild(
                            GlobalStatic.PageLayout.DomThis
                        );
                    } else {
                        GlobalStatic.app.DomThis.replaceChild(
                            GlobalStatic.PageLayout.DomThis,
                            this.CurrentPageLayoutDom
                        );
                    }
    ...
    private CreatePage<T extends HeosabiComponent>(
        Component: new () => T,
        match: NavigateMatchModel
    ) {
        if (
            null === GlobalStatic.PageNow ||
            ...
            GlobalStatic.PageNow.DomThisComplete = () => {
                if (null == this.CurrentPageDom) {
                    ...
                    GlobalStatic.PageLayout.Body.appendChild(
                        GlobalStatic.PageNow.DomThis
                    );
                } else {
                    GlobalStatic.PageNow.RouteThis = match;
                    GlobalStatic.PageLayout.Body.replaceChild(
                        GlobalStatic.PageNow.DomThis,
                        this.CurrentPageDom
                    );
                }
